...
function normalize(str){return String(str).trim().replace(/[.!?]+$/,'').replace(/\s+/g,' ').toLowerCase();}
...
async function renderLesson(){
  ...
  function renderCurrent(){updateHUD(); feedback.classList.add('hidden'); nextBtn.classList.add('hidden'); checkBtn.classList.remove('hidden'); currentItem=lesson.items[current]; selectedChoice=null; scrambleAnswer=[]; matchSelections={};
    ...
    else if(currentItem.type==='info'){playBtn.style.display='none'; box.innerHTML=`<div class="info-card"><h3>${currentItem.title||'Study'}</h3><p>${String(currentItem.body||'').replaceAll('\n','<br>')}</p></div>`;}
    else if(currentItem.type==='translation'){playBtn.style.display='none'; box.innerHTML='<p>Translate into English.</p><div class="translation-prompt">'+currentItem.prompt+'</div><input id="answerInput" type="text" placeholder="Type the English sentence" autocomplete="off">'; setTimeout(()=>document.getElementById('answerInput')?.focus(),30);}
  ...
  checkBtn.addEventListener('click',()=>{let ok=false, correctAnswer='';
    if(currentItem.type==='info'){ok=true; correctAnswer='Continue.';}
    else if(currentItem.type==='dictation'){const val=document.getElementById('answerInput')?.value||''; ok=normalize(val)===normalize(currentItem.text); correctAnswer=currentItem.text;}
    else if(currentItem.type==='translation'){const val=document.getElementById('answerInput')?.value||''; ok=normalize(val)===normalize(currentItem.answer); correctAnswer=currentItem.answer;}
